...
{
    public static class StaticGenericPropertyAccessor<T>
    {
        private static readonly ConcurrentDictionary<string, Func<T, object>> _cachedGetMethods
            = new ConcurrentDictionary<string, Func<T, object>>();

        private static readonly ConcurrentDictionary<string, Action<T, object>> _cachedSetMethods
        ...
        private static Action<T, object> CreateSetMethod(string propertyName)
        {
        ...
        private static Func<T, object> CreateGetMethod(string propertyName)
        {
            ParameterExpression objParamExpr = Expression.Parameter(typeof(T));
